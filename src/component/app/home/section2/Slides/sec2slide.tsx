...
export default function Sec2Slide() {
  const [add, setAdd] = useState(false);

  // انیمیشن‌های مشترک
  const cardVariants = {
    hidden: { opacity: 0, y: 50 },     // حالت اولیه: پایین و نامرئی
    visible: { opacity: 1, y: 0 },     // حالت نهایی: در جای اصلی
  };

  return (
    <motion.div
      className="h-90 flex flex-col items-center justify-center rounded-lg px-5"
      // انیمیشن اولیه (وقتی صفحه لود می‌شود)
      initial="hidden"
      animate="visible"
      variants={cardVariants}
      transition={{ duration: 0.5, delay: 0.2, ease: "easeOut" }}
      // انیمیشن اسکرول (وقتی کارت وارد viewport شود)
      whileInView="visible"
      viewport={{ once: true, amount: 0.3 }} // once: فقط یکبار, amount: چه نسبتی از کارت دیده شود
    >
      <div className="w-full h-full flex flex-col">
        {/* بخش تصویر */}
        <motion.div
          className="bg-gray-200 w-full h-14/20 relative rounded-xl overflow-hidden cursor-pointer"
          whileHover={{ scale: 1.03 }}    // زوم مدرن در هاور
          transition={{ type: "spring", stiffness: 300 }}
          onMouseEnter={() => setAdd(true)}
            onMouseLeave={() => setAdd(false)}
        >
          <Image
            src={"/Home/Frame611.svg"}
            alt={"sad"}
            width={250}
            height={277}
